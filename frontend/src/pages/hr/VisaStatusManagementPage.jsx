...
export default function VisaStatusManagementPage() {
  ...
  const handleViewPendingDocument = async (employeeId, docId) => {
    ...
    if (!res.error && res.payload) {
      ...
      if (doc) {
        ...
        setTimeout(() => {
          const approvalElement = document.querySelector('.document-approval-card');
          if (approvalElement) {
            approvalElement.scrollIntoView({ behavior: 'smooth', block: 'nearest' });
          }
        }, 300);
      }
    }
  ...
  const handleApprove = async ({ docId, hrFeedback }) => {
    ...
    if (res.error) {
      message.error(res.error.message || 'Approval failed');
    } else {
  ...
  const handleReject = async ({ docId, hrFeedback }) => {
    ...
    if (res.error) {
      message.error(res.error.message || 'Rejection failed');
    } else {
      ...
      children: (
        <div className={styles.inProgressContainer}>
          <Row gutter={[16, 16]} className={styles.contentRow}>
            <Col xs={24} className={styles.listColumn}>
              <InProgressList
            ...
            {selectedDocs.length > 0 && (
              <Col xs={24} className={styles.actionColumn}>
